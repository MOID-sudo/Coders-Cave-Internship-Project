...
class ExpenseSharingApp:
    ...
    def create_widgets(self):
        ...
        tk.Label(input_frame, text="Description:", bg='#f5f5f5').grid(row=0, column=0, sticky="e")
    ...
    def add_expense(self):
        ...
        if not description or not paid_by or not shared_with or not date or not category:
    ...
    def update_expenses_listbox(self):
        ...
        for expense in self.expenses:
            self.expenses_listbox.insert(
                tk.END, 
                f"{expense['description']} ({expense['date']}) - {expense['category']}: {expense['amount']} paid by {expense['paid_by']} shared with {', '.join(expense['shared_with'])}"
            )
